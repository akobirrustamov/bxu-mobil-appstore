...
const InProgress = () => {
    ...
    const filterCommands = () => {
        ...
        if (selectedStatus != 0 && selectedStatus !=3) {
            filtered = filtered.filter((cmd) => cmd.status?.toString() === selectedStatus.toString());
        }
        if (selectedStatus == 3) {
            filtered = filtered.filter((item) => {
                const now = new Date();
                const tileLimitDate = new Date(item.tileLimit);
                const timeDifferenceInMs = tileLimitDate - now;
                return timeDifferenceInMs < 0;
            });
        }
    ...
    const renderCommandItem = ({ item, index }) => {
        ...
        const timeDifferenceInMs = tileLimitDate - now; // Time difference in milliseconds
        const timeDifferenceInHours = timeDifferenceInMs / (1000 * 60 * 60); // Convert to hours
        const timeDifferenceInDays = Math.floor(timeDifferenceInHours / 24); // Days
        const remainingHours = Math.floor(timeDifferenceInHours % 24); // Remaining hours

        let circleColor = "red";
        let timeText = `Qolgan vaqt: ${Math.abs(timeDifferenceInDays)} kun ${Math.abs(remainingHours)} soat`;

        // If time difference is negative, show overdue message
            ...
            <View style={styles.commandCard}>
                ...
                <View style={styles.checked}>
                    ...
                    <View style={styles.checkIcons}>
                        ...
                        {item.status === 2 && <FontAwesome name={"check"} style={styles.myCheck} />}
                    </View>
                ...
                >
                    ...
                    <Text style={{ color: "#6200EE"}}>Batafsil..</Text>
                </TouchableOpacity>
