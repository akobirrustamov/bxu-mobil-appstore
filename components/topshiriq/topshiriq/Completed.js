...
const Completed = () => {
    ...
    const getMyCommands = async () => {
        try {
            ...
            const response = await ApiCall(`/api/v1/app/staff/my-commands/${token}/4`, "GET");
            if (response.status === 200 && response.data) {
    ...
    const renderCommandItem = ({ item, index }) => {
        ...
        return (
            ...
            <View style={styles.commandCard} key={item.id}>
                ...
                <Text style={styles.commandInfo}>
                    <Text style={styles.label}>
                        <FontAwesome name={"clock-o"} style={styles.myCheck} /> Topshiriq tugatilgan sana:
                    </Text>{" "}
                    {new Date(item.responseTime).toLocaleDateString("en-GB")},{" "}
                    {new Date(item.responseTime).toLocaleTimeString("en-GB", {
                        hour: "2-digit",
                        minute: "2-digit",
                    })}
                </Text>
